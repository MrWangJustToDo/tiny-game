          ...
          .promiseNext(0, () => {
            for (let i = 0; i < 3; i++) {
              for (let j = 0; j < 3; j++) {
                ...
                cover.style.width = "10px";
                cover.style.height = "10px";
                cover.style.left = `${j * 10}px`;
                cover.style.top = `${i * 10}px`;
  ...
  class Game {
    ...
    clearAll() {
      return tools.promiseNext(this.shortDelay, () => {
        this._resetTime();
        Array.from(document.querySelectorAll("." + this.rowClassName)).forEach(
          (it) => it.remove()
        );
      });
    }
...
(() => {
  addEventListener("DOMContentLoaded", () => {
    ...
    let options = {
      ...
      targetUrl: "url(./images/target.jpg)",
      baseSize: 30,
